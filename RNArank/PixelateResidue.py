"""
https://github.com/lijunRNA/RNA3DCNN/blob/master/PixelateResidue.py
"""
...
TRIAL_HALF_BOX_SIDE_LENGTH = HALF_BOX_SIDE_LENGTH + 5.0 #TRIAL
BIN_WIDTH = 1.0
...
def check_if_lacking_atoms(res, atomname):
    if not res.has_id(atomname):
        print('There is no atom ' + atomname + ' in residue ' + str(res.get_id()[1]) + res.get_resname()[2] + ' in chain ', res.get_full_id()[2] + ' in PDB ' + res.get_full_id()[0] + '.')
        return True
    else:
        return False
...
def lattice_1d_point(dis, int_coord, prob):
    ...
    prob[0] = ((int_coord[0] + 1) * BIN_WIDTH + HALF_BIN_WIDTH - dis) / BIN_WIDTH
...
def lattice_3d_point(vec_3d, int_coord_x, int_coord_y, int_coord_z, prob_x, prob_y, prob_z):
...
def is_neighbor_residue(local_ref, ori2):
    vec_r = Vector(ori2) - local_ref[0]
    if abs(vec_r * local_ref[1]) > TRIAL_HALF_BOX_SIDE_LENGTH: # TRIAL_HALF_BOX_SIDE_LENGTH = 21
        return False
...
def pixelate_atoms_in_box(model, pixels, xyz_ori, frame_C4_):
    ...
    prob_y = [0, 0]
    prob_z = [0, 0]
    for res_index1, residue1 in enumerate(residues):
        if res_index1 >= xyz_ori.shape[0] or res_index1 >= frame_C4_.shape[0]:
            break
        ori = Vector(xyz_ori[res_index1, :])
        vec_x = Vector(frame_C4_[res_index1,:,0])
        vec_y = Vector(frame_C4_[res_index1, :, 1])
        vec_z = Vector(frame_C4_[res_index1, :, 2])
        local_ref = [ori, vec_x, vec_y, vec_z]
        for res_index2, residue2 in enumerate(residues):
            if res_index2 >= xyz_ori.shape[0]:
                break
            if not is_neighbor_residue(local_ref=local_ref, ori2=xyz_ori[res_index2, :]):
                continue
            res_name = residue2.get_resname().strip()
            for atom in residue2:
                ...
                if not is_neighbor_atom(local_ref, atom, local_coordinate):
                    continue
                lattice_3d_point(local_coordinate, int_coord_x, int_coord_y,int_coord_z, prob_x, prob_y, prob_z)
                    ...
                    for j in range(2):
                        ...
                        for k in range(2):
                            if int_coord_z[k] < 0 or int_coord_z[k] >= NBINS:
                                continue
                            pixels[res_index1][0][int_coord_x[i]][int_coord_y[j]][int_coord_z[k]] += prob_x[i] * prob_y[j] * prob_z[k]
                            pixels[res_index1][1][int_coord_x[i]][int_coord_y[j]][int_coord_z[k]] += prob_x[i] * prob_y[j] * prob_z[k] * atom_mass
                            pixels[res_index1][2][int_coord_x[i]][int_coord_y[j]][int_coord_z[k]] += prob_x[i] * prob_y[j] * prob_z[k] * atom_charge
